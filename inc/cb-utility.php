...
add_shortcode(
    'registered_address',
    function () {
        $output = get_field( 'registered_address', 'option' );
        return $output;
    }
);
...
add_shortcode(
    ...
    function ( $atts ) {
        ...
        if ( get_field( 'contact_phone_2', 'option' ) ) {
            $phone       = get_field( 'contact_phone_2', 'option' );
            $icon_html   = $atts['icon'] ? '<i class="fa-solid fa-phone"></i> ' : '';
            $anchor_text = $icon_html . ( ! empty( $atts['text'] ) ? wp_kses_post( $atts['text'] ) : esc_html( $phone ) );
            $class       = esc_attr( $atts['class'] );
...
add_action(
	...
	function () {
		wpcf7_add_form_tag(
			...
			'cb_cf7_honeypot_form_tag_handler',
			array(
				'name-attr'    => true,
				'do-not-store' => true,
			)
 ...
 */
function cb_cf7_honeypot_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
...
add_filter( 'wpcf7_validate_honeypot', 'cb_cf7_honeypot_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_honeypot*', 'cb_cf7_honeypot_validation_filter', 10, 2 );
 ...
 */
function cb_cf7_honeypot_validation_filter( $result, $tag ) {
	$name = $tag->name;
